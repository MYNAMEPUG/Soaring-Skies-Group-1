...
def log_custom(msg, important=False):
    ...
    print(
        f'{(bcolors.FAIL + "[IMPORTANT] ") if important else bcolors.YELLOW}{bcolors.UNDERLINE}CUSTOM{bcolors.ENDC} > {msg}')
...
def mission_log(msg):
    ...
    if msg.msgname == 'MISSION_ACK':
        print(
            f'{bcolors.MISSION}{msg.msgname}{bcolors.ENDC} - {(bcolors.OKGREEN + "UPLOADED") if msg.type == 0 else (bcolors.FAIL + "NOT UPLOADED")}{bcolors.ENDC} > {msg}')
    else:
