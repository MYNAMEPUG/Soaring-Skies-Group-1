...
class Mission:
    ...
    def mission_setup(self, waypoints):
        ...
        connection.mav.mission_clear_all_send(connection.target_system, connection.target_component,
                                              common.MAV_MISSION_TYPE_MISSION)
    ...
    async def get_mission_items(self, waypoints: list = None):
        ...
        encoded_commands = [connection.mav.mission_item_int_encode(
            connection.target_system, connection.target_component, i, common.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,
            common.MAV_CMD_NAV_TAKEOFF, 1, 1,
            0, 0, 0, 0, int(home_pos['lat']), int(home_pos['long']), self.drone.target_alt,
            common.MAV_MISSION_TYPE_MISSION
        )]
        ...
        for wp in waypoints:
            ...
            encoded_commands.append(connection.mav.mission_item_int_encode(
                connection.target_system, connection.target_component, i, common.MAV_FRAME_GLOBAL_RELATIVE_ALT,
                common.MAV_CMD_NAV_WAYPOINT, 0, 1,
    ...
    async def upload_mission(self, waypoints: list = None, rtl: bool = True, begin_immediately=False):
        ...
        Once received, it sends each encoded mission item, every time a MISSION_REQUEST is received. After all mission items sent, function waits for a MISSION_ACK"""
        ...
        log_system(msgname=DroneProperties.MISSION_HANDLER,
                   msg=f"Sending mission type MISSION count of {self.mission_length}")
        connection.mav.mission_count_send(connection.target_system, connection.target_component, self.mission_length,
                                          common.MAV_MISSION_TYPE_MISSION)

        for i in range(self.mission_length):
            request = await self.drone.get_message_stream().wait_for_message('MISSION_REQUEST', 'MISSION_REQUEST_INT',
                                                                             secondary=common.MAV_MISSION_TYPE_MISSION)
        ...
        mission_ack = await self.drone.get_message_stream().wait_for_message("MISSION_ACK",
                                                                             secondary=common.MAV_MISSION_TYPE_MISSION)
        log_system(msgname=DroneProperties.MISSION_HANDLER, msg=f"Received Mission Ack: {mission_ack}",
                   color=bcolors.MISSION)
