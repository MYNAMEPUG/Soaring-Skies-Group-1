...
class MessageStream:
    ...
    async def send_command(self, command,
                           param1=0, param2=0, param3=0, param4=0, param5=0, param6=0, param7=0, timeout=3, retries=3):
        ...
        self.connection.mav.command_long_send(self.connection.target_system, self.connection.target_component, command,
                                              0,
        ...
        try:
            ...
            # log_custom(f'received command ack code: {command}')
        except TimeoutError:
            ...
            if retries >1:
                ...
                log_system(color=bcolors.FAIL,msg=f"COMMAND ({command}) TIMED OUT TOO MANY TIMES")
            return

        if ack.result != 0:
            log_system(msg="PASSING INTO RESULT HANDLER...STANDBY", msgname="ACK_HANDLER")
            await self.result_handler(ack, command, param1, param2, param3, param4, param5, param6, param7,retries=retries)
        return ack  # no harm in return the command_ack

    async def send_command_int(self, command,
                               param1=0, param2=0, param3=0, param4=0, x=0, y=0, z=0, timeout=5,
                               frame: int|bool=common.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT, retries: int = 3):
        ...
        self.connection.mav.command_int_send(self.connection.target_system, self.connection.target_component, frame,
                                             command, 0, 0,
        ...
        try:
            ...
            log_system(color=bcolors.FAIL, msg=f"FAILED TO RECEIVE COMMAND_ACK ({command})")
            if retries > 1:
                await self.send_command_int(command, param1, param2, param3, param4, x, y, z,
                                            timeout=timeout, frame=frame, retries=retries - 1)
            else:
                log_system(color=bcolors.FAIL, msg=f"COMMAND ({command}) TIMED OUT TOO MANY TIMES")
            return None
        ...
        if ack.result != 0:
            log_system(msg="PASSING INTO RESULT HANDLER...STANDBY", msgname="ACK_HANDLER")
            await self.result_handler(ack, command, param1, param2, param3, param4, x, y, z, frame=frame,retries=retries)
        return ack  # no harm in return the command_ack

    async def result_handler(self, ack, command,
                             param1=0, param2=0, param3=0, param4=0, param5=0, param6=0, param7=0, retries: int = 3,
                             frame=False):
        match ack.result:
            case 1:  # MAV_RESULT_TEMPORARILY_REJECTED
                log_system(msg="MAV RESULT TEMP REJECTED. RETRYING COMMAND IN 1 SECOND",
                           msgname="ACK_HANDLER (RETRYING)")
                ...
                if (retries > 1):
                    if frame is False:
                        ...
                        await self.send_command_int(command,
                                                    param1, param2, param3, param4, param5, param6, param7,
                                                    retries=retries - 1, frame=frame)
                else:
                    log_system(msg='COMMAND REJECTED TOO MANY TIMES', color=bcolors.FAIL)
            case 2:  # MAV_RESULT_DENIED
                log_system(msgname="ACK_HANDLER (ERROR)", color=bcolors.FAIL,
                           msg="MAV RESULT FAILED. ONE OR MORE PARAMETERS INVALID")
    ...
    async def handle_message(self, msg, retries=5):
