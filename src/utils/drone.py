...
class Drone:
    def __init__(self):
        ...
        self.climbspeed = None
        self.threshholdZ = None
    ...
    def retrieve_configuration(self, parametersFile=None, configFile=None):
        ...
        self.gimbal = True if (parametersFile and params["gimbal"] == "True") else False

        if configFile:
            self.config = read_config_file(configFile)
            print(self.config)

    def connect_drone(self, connection_string: str='tcp:127.0.0.1:5763', port: str|bool=False):
        ...
        log_system(
            msg=f"{bcolors.HEADER} -- Connected to {connection_string} on port {port if port else 'NO PORT'} with system id {self.connection.target_system} and component id {self.connection.target_component} --{bcolors.ENDC}",
    ...
    def initialize_drone(self, parametersFile, configFile):
        ...
        try:
            if self.connection_str.__contains__(':'):
                self.port = self.connection_str.split(':')[-1]
            else:
                self.port = False
        except Exception:
            print('exception idk in initialize_drone() in drone.py')
    ...
    async def send_heartbeat(self):
        while True:
            ...
            log_system(msgname=f"{DroneProperties.HEARTBEAT_SEND}", msg="Sent heartbeat to drone from GCS",
                       color=bcolors.HEARTBEAT)
            await asyncio.sleep(1)

    async def send_command(self, command: int, param1=0, param2=0, param3=0, param4=0, param5=0, param6=0, param7=0,
                           timeout=5):
        ...
        return await self.message_stream.send_command(command=command, param1=param1, param2=param2, param3=param3,
                                                      param4=param4,
                                                      param5=param5, param6=param6, param7=param7, timeout=timeout)

    async def send_command_int(self, command: int, param1=0, param2=0, param3=0, param4=0, param5=0, param6=0, param7=0,
                               timeout=5, frame=common.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT):
        ...
        return await self.message_stream.send_command_int(command=command, param1=param1, param2=param2, param3=param3,
                                                          param4=param4,
                                                          param5=param5, param6=param6, param7=param7, timeout=timeout,
                                                          frame=frame)
    ...
    async def watch_for_heartbeat(self):
        ...
        await self.message_stream_exists(msgname='Heartbeat Watcher',
                                         msg="Message Stream not yet initialized | Awaiting Message Stream activation")
        retries = 5
        while True:
            if retries == 0:
                log_system(msg="Drone connection lost...", msgname="Heartbeat Watcher (CRITICAL FAILURE)",
                           color=bcolors.FAIL)
                break
            try:
                hb = await asyncio.wait_for(self.message_stream.wait_for_message("HEARTBEAT"), timeout=3.0)
                # print('heartbeat recieved from drone')
                retries = 5
            except TimeoutError:
                log_system(msgname="Heartbeat Watcher (URGENT)",
                           msg=f"Too long since last heartbeat recieved. {retries - 1} retries until failsafe activation",
                           color=bcolors.HEARTBEAT)
                retries -= 1
        i = 1
        while True:
            log_system(msg=f"Heartbeats missed: {i}", msgname="Heartbeat Watcher (CRITICAL FAILURE)",
                       color=bcolors.FAIL)
            i += 1
            await asyncio.sleep(1)
    ...
    async def set_home_position(self):
        await self.message_stream_exists(msgname="Home Position",
                                         msg="Message Stream not yet initialized | Awaiting Message Stream activation")
        ...
        while True:
            ...
            if pos_msg.lat != 0 and pos_msg.lon != 0:
                break
            await asyncio.sleep(0.5)
        home_pos: HomePosition = {'lat': pos_msg.lat, 'long': pos_msg.lon, 'alt': (pos_msg.relative_alt / 1000), 'set': True}
    ...
    async def get_home_position_deg(self):
        src= await self.get_home_position_int()
        home_pos: HomePosition={'lat':src['lat']/1e7,'long': src['long'] / 1e7,'alt': src['alt'],'set': src['set']}
        return home_pos
    ...
    async def get_position_deg(self):
        ...
        return {"lat": current_pos.lat, "long": current_pos.lon,
                "alt": (self.unit_corrector(current_pos.relative_alt / 1000))}
