...
from utils.takeoff import arm_drone
from utils.logger import log_system, bcolors


async def test_all_motors(drone: Drone, throttle_value: int = 10):
    log_system(msgname="MOTOR TEST INITIATING",
               msg="Motor test beginning in 3 seconds. Make sure all props have been removed. " \
                   "Kill this program if not.", color=f'{bcolors.BOLD}{bcolors.UNDERLINE}{bcolors.OKCYAN}')

    await asyncio.sleep(3)

    log_system(msg="BEGINNING MOTOR TEST", msgname="MOTOR TEST INITIATING",
               color=f'{bcolors.BOLD}{bcolors.UNDERLINE}{bcolors.OKCYAN}')
...
async def test_all_motors_multiple(drone: Drone, num_times: int = 1, throttle_value: int = 20, throttle_step: int = 0):
