...
crop_output_dir = Path("saved_crops")
crop_output_dir.mkdir(exist_ok=True)

saved_one_crop = False
...
for result in detector_results:
    ...
    for box in result.boxes:
        ...
        # Save one example crop for checking
        if not saved_one_crop:
            crop_save_path = crop_output_dir / f"{img_path.stem}_{shape_name}_crop.jpg"
            cv2.imwrite(str(crop_save_path), crop)
            saved_one_crop = True
            print(f"Saved example crop to: {crop_save_path}")
...
print("One example crop saved in saved_crops/")